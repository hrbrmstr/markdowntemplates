...
prismskel <- function(number_sections = FALSE,
  ...
  pre_processor <- function(metadata, input_file, runtime,
    ...
    args <- c()
    args <- c(args, pandoc_html_highlight_args(
      highlight,
      template, self_contained, lib_dir,
      output_dir
    ))
    args <- c(args, includes_to_pandoc_args(
      includes = includes,
      filter = if (identical(runtime, "shiny")) normalize_path else identity
    ))
    args
  ...
  output_format(
    knitr = rmarkdown::knitr_options(
      opts_knit = list(keep.md = keep_md),
      opts_chunk = list(
        fig.width=fig_width, fig.height=fig_height,
        fig.retina=fig_retina, dev=dev
      ),
      knit_hooks = list(
        chunk = chunk_hook_html,
